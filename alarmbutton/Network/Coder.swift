...
class Coder {
    ...
    private let bytesPt2: [Byte] = [
        0xAE, 0xD4, 0xF8, 0x44, 0x05, 0x83, 0x75, 0xEA, 0x3B, 0x4F, 0x3F, 0xB3, 0x22, 0x65, 0xEB, 0x4D,
    ...
    func encode(text: String) -> Data {
        ...
        for index in 2...20 {
            encoded[index] = encoded[index] ^ bytesPt1[Int(indexA)] ^ bytesPt2[Int(indexB)]
        ...
        for index in 0...(text.count - 1) {
            encoded[headerSize + 4 + index] ^= bytesPt1[Int(indexA)] ^ bytesPt2[Int(indexB)]
    ...
    func decode(data: Data) -> String? {
        ...
        for index in 2...20 {
            header[index] = header[index] ^ bytesPt1[Int(indexA)] ^ bytesPt2[Int(indexB)]
        ...
        for index in 0...(bufferSize - 3) {
            content[headerSize + 4 + Int(index)] ^= bytesPt1[Int(indexA)] ^ bytesPt2[Int(indexB)]
    ...
    private func byteArrayFrom(uint32: UInt32) -> [Byte] {
        ...
        for index in 0...3 {
            let shift = (3 - index) * 8
            bytes[index] = Byte(uint32 >> shift & UInt32(Byte.max))
        }
